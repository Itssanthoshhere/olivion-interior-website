      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="w-full lg:w-2/3">
          <h1 className="CalSans text-4xl md:text-7xl mb-5 w-full lg:w-[80%]">
            Take A look At{" "}
            <span className="text-(--primary)"> Our Latest Blog </span> &amp;
            Articles.
          </h1>
        </div>
      </div>
            ...
            <div key={index} className="group cursor-pointer">
              <Link href={`/UI-Components/Blogs/${blog.id}`}>
                <div className="blog-image relative rounded-2xl overflow-hidden">
                  ...
                  <span className="absolute top-5 left-5 bg-(--primary) px-4 py-1 font-semibold rounded-full text-white GolosText">
                    {blog.tag}
                  </span>
                </div>
                ...
                <div className="blog-info mt-3">
                  ...
                  <h2 className="text-4xl md:text-5xl CalSans my-5 hover:text-(--primary) transition-all duration-300">
                    {blog.title}
                  </h2>
              ...
              <Link href={`/UI-Components/Blogs/${blog.id}`}>
                <div className="flex flex-col md:flex-row group cursor-pointer mb-5 gap-4">
                  <div className="w-full lg:w-1/2 blog-image relative rounded-2xl overflow-hidden">
                    ...
                    <span className="absolute top-3 left-2 bg-(--primary) px-4 py-1 font-semibold rounded-full text-white GolosText">
                      {blog.tag}
                    </span>
                  </div>
                  ...
                  <div className="w-full lg:w-1/2 blog-info mt-3">
                    ...
                    <h2 className="text-3xl md:text-2xl CalSans my-3 hover:text-(--primary) transition-all duration-300">
                      {blog.title}
                    </h2>
                    <p className="text-gray-500 GolosText">{blog.desc}</p>
                  </div>
