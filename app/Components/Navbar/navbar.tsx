            ...
            {navLinks.map((link) =>
              link.dropdown ? (
                <div key={link.label} className="relative group z-50">
                  <Link
                    href={link.href}
                    className="flex menu-links text-xl items-center gap-1 hover:text-(--primary) transition-all duration-300"
                  >
                ...
                <Link
                  key={link.label}
                  href={link.href}
                  className="text-xl hover:text-(--primary) transition-all duration-300"
                >
                  {link.label}
                </Link>
              )
        ...
        <div className="flex items-center gap-4">
          <button className="hidden lg:flex items-center gap-1">
            ...
            <div className="flex flex-col items-start">
              <p>Call Us Now</p>
              <h3 className="text-(--primary) GolosText">+91 (123) 456 789</h3>
            </div>
          </button>

          <Link href="/app/UI-Components/Pages/Contact">
              ...
              {link.dropdown ? (
                <>
                  <button
                    ...
                    className="w-full flex justify-between items-center px-4 py-3 text-left text-(--text) font-medium hover:text-(--primary) transition"
                  >
                    {link.label}
                    <i
                  ...
                  >
                    {link.dropdown.map((item) => (
                      <Link
                        key={item.label}
                        href={item.href}
                        className="block py-2 font-semibold hover:text-(--primary) transition border-b border-gray-700/60"
                        onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                      >
                        {item.label}
                      </Link>
                ...
                <Link
                  href={link.href}
                  className="block px-4 py-3 text-(--text) hover:text-(--primary-color) transition font-medium"
                  onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                >
                  {link.label}
                </Link>
